...
const Auth = {
  user()    { try { return JSON.parse(sessionStorage.getItem('slm_user')); } catch { return null; } },
  setUser(u){ if(u) sessionStorage.setItem('slm_user',JSON.stringify(u)); else { sessionStorage.removeItem('slm_user'); } },
  require(roles=null) {
    const u = this.user();
  ...
  async register(data, inviteToken=null) {
    ...
    let invite = null;
    if (inviteToken) {
      const rows = await SB.select('invitations', {token:`eq.${inviteToken}`, status:'eq.pending', select:'id,role,invited_by,expires_at'});
      invite = rows?.[0] || null;
      ...
      const rows = await SB.select('invitations', {email:`eq.${email}`, status:'eq.pending', select:'id,role,invited_by'});
      invite = rows?.[0] || null;
    }
    ...
    const user   = await SB.insert('users', {
      email, password_hash: pwHash, role,
      first_name: data.first_name.trim(), last_name: data.last_name.trim(),
      company: data.company.trim(), phone: data.phone.trim(),
      siret: data.siret?.trim()||'', address: data.address?.trim()||'',
      parent_id: invite?.invited_by || null, status,
    });
    if (invite) await SB.update('invitations', {id:`eq.${invite.id}`}, {status:'used', used_at: new Date().toISOString()});
  ...
  async updateProfile(userId, data) {
    ...
    const update = {};
    allowed.forEach(k => { if (data[k] !== undefined && data[k] !== '') update[k] = data[k]; });
  ...
  async createInvitation(email, role, invitedBy, note='') {
    ...
    const inv = await SB.insert('invitations', {
      email, token, role, invited_by: invitedBy, note: note||'', status:'pending',
